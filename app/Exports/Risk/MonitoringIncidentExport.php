        ...
        $this->worksheets->each(function ($worksheet) use (&$data, $answers) {
            $worksheet->monitorings->each(function ($monitoring) use ($worksheet, &$data, $answers) {
                ...
                $data[] = [
                    ...
                    'Nama Kejadian' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->incident_body ?? ''), ['ignore_errors' => true])),
                    'Identifikasi Kejadian ' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->incident_identification ?? ''), ['ignore_errors' => true])),
                    'Kategori Kejadian' => $monitoring->incident?->incident_category?->name ?? '',
                    'Sumber Penyebab Kejadian' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->incident_source ?? ''), ['ignore_errors' => true])),
                    'Penyebab Kejadian' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->incident_cause ?? ''), ['ignore_errors' => true])),
                    'Penanganan saat Kejadian' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->incident_handling ?? ''), ['ignore_errors' => true])),
                    'Deskripsi Kejadian - Risk Event' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->incident_description ?? ''), ['ignore_errors' => true])),
                    'Kategori Risiko Perusahaan' => $riskCategory,
                    'Penjelasan Kerugian' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->loss_description ?? ''), ['ignore_errors' => true])),
                    ...
                    'Mitigasi yang Direncanakan' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->mitigation_plan ?? ''), ['ignore_errors' => true])),
                    'Realisasi Mitigasi' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->actualization_plan ?? ''), ['ignore_errors' => true])),
                    'Perbaikan Mendatang' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->follow_up_plan ?? ''), ['ignore_errors' => true])),
                    'Pihak terkait' => str_replace('\n', '\r\n', Html2Text::convert(html_entity_decode($monitoring->incident?->related_party ?? ''), ['ignore_errors' => true])),
