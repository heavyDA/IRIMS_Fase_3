...
const createDatatable = (target = 'table', options) => {
        ...
        if (type === 'select') {
            ...
            element.addEventListener('keydown', debounce(() => {
                if (column.search() !== element.value) {
                    column.search(operator ? operator + '___' + element.value : element.value).draw()
                }
            }))
    ...
    const setColumnSearchField = function () {
        ...
        if (column.searchable()) {
            const data = column.fieldType() === 'select' ? column.fieldTypeData() : [];
            const input = createColumnSearchField(column, column.fieldType(), column.searchOperator(), data);
