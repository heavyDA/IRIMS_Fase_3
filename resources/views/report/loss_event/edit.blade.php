...
@section('header-content')
    <div>
        <h1 class="page-title fw-medium fs-18 mb-2">Form Edit Loss Event Database</h1>
        <div class="">
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item">Risk Report</li>
                    <li class="breadcrumb-item"><a href="{{ route('risk.report.loss_events.index') }}">Loss Event Database</a>
                    </li>
...
@section('main-content')
    <x-card>
        <x-slot name="body">
            <form id="loss-event-form" method="POST"
                action="{{ route('risk.report.loss_events.update', $lossEvent->getEncryptedId()) }}" class="row gap-2">
                @csrf
                @method('PATCH')
                <div class="row mb-1">
                    <div class="col-3">
                    ...
                    <div class="col">
                        <select name="worksheet_id"
                            ...
                            @foreach ($worksheets as $worksheet)
                                <option {{ $lossEvent->worksheet_id == $worksheet->id ? 'selected' : null }}
                                    value="{{ $worksheet->id }}">
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        ...
                        <textarea type="text" name="incident_body" class="d-none {{ $errors->has('incident_body') ? 'is-invalid' : null }}">{{ $lossEvent->incident_body }}</textarea>
                        @error('incident_body')
                            <x-forms.error :message="$message"></x-forms.error>
                        @enderror
                    </div>
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        <input type="datetime-local"
                            ...
                            name="incident_date"
                            value="{{ format_date($lossEvent->incident_date->toDateTimeString())->toDateTimeString() }}">
                        @error('incident_date')
                            <x-forms.error :message="$message"></x-forms.error>
                        @enderror
                    </div>
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        ...
                        <textarea type="text" name="incident_source"
                            class="d-none {{ $errors->has('incident_source') ? 'is-invalid' : null }}">{{ $lossEvent->incident_source }}</textarea>
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        ...
                        <textarea type="text" name="incident_handling"
                            class="d-none {{ $errors->has('incident_handling') ? 'is-invalid' : null }}">{{ $lossEvent->incident_handling }}</textarea>
                    ...
                    <div class="col">
                        <select class="form-select {{ $errors->has('risk_category_t2_id') ? 'is-invalid' : null }}"
                            ...
                            @foreach ($risk_categories as $parent)
                                <option {{ $lossEvent->risk_category_t2_id == $parent->id ? 'selected' : null }}
                        ...
                        <select class="form-select {{ $errors->has('risk_category_t3_id') ? 'is-invalid' : null }}"
                            ...
                            @foreach ($risk_categories as $parent)
                                @foreach ($parent->children as $child)
                                    <option {{ $lossEvent->risk_category_t3_id == $child->id ? 'selected' : null }}
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        <input type="text" class="form-control {{ $errors->has('loss_value') ? 'is-invalid' : null }}"
                            name="loss_value_format" value="{{ $lossEvent->loss_value }}">
                        <input type="number" step=".01" class="d-none" name="loss_value"
                            value="{{ $lossEvent->loss_value }}">
                        @error('loss_value')
                            <x-forms.error :message="$message"></x-forms.error>
                        @enderror
                    </div>
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        ...
                        <textarea type="text" name="related_party" class="d-none {{ $errors->has('related_party') ? 'is-invalid' : null }}">{{ $lossEvent->related_party }}</textarea>
                        @error('related_party')
                            <x-forms.error :message="$message"></x-forms.error>
                        @enderror
                    </div>
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        ...
                        <textarea type="text" name="restoration_status"
                            class="d-none {{ $errors->has('restoration_status') ? 'is-invalid' : null }}">{{ $lossEvent->restoration_status }}</textarea>
                        ...
                        <div class="d-flex gap-4 {{ $errors->has('insurance_status') ? 'is-invalid' : null }}">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="insurance_status"
                                    id="insurance_status_true" {{ $lossEvent->insurance_status ? 'checked' : null }}
                                    value="1">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="insurance_status"
                                    id="insurance_status_false" {{ !$lossEvent->insurance_status ? 'checked' : null }}
                                    value="0">
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        <input type="text"
                            ...
                            name="insurance_permit_format" value="{{ $lossEvent->insurance_permit }}">
                        <input type="number" step=".01" class="d-none" name="insurance_permit"
                            value="{{ $lossEvent->insurance_permit }}">
                ...
                <div class="row mb-1">
                    ...
                    <div class="col">
                        <input type="text"
                            ...
                            name="insurance_claim_format" value="{{ $lossEvent->insurance_claim }}">
                        <input type="number" step=".01" class="d-none" name="insurance_claim"
                            value="{{ $lossEvent->insurance_claim }}">
