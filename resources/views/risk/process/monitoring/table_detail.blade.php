                        ...
                        <div class="d-flex flex-column" style="min-height: 300px">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane border-0 p-2 show active" id="tableResidual" role="tabpanel">
                                    @include('risk.process.monitoring.table_detail._residual')
                                </div>
                                <div class="tab-pane border-0 p-2" id="tableActualization" role="tabpanel">
                                    @include('risk.process.monitoring.table_detail._actualization')
                                </div>
                                <div class="tab-pane border-0 p-2" id="tableAlteration" role="tabpanel">
                                    @include('risk.process.monitoring.table_detail._alteration')
                                </div>
                                <div class="tab-pane border-0 p-2" id="tableIncident" role="tabpanel">
                                    @include('risk.process.monitoring.table_detail._incident')
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            <div class="d-flex gap-2">
                                @if (str_contains(request()->route()->getName(), 'create') || str_contains(request()->route()->getName(), 'show'))
                                    <a href="{{ route('risk.process.monitoring.show', $worksheet->getEncryptedId()) }}"
                                ...
                                @elseif(str_contains(request()->route()->getName(), 'edit'))
                                    <a href="{{ route('risk.process.monitoring.show_monitoring', $monitoring->getEncryptedId()) }}"
                                        style="min-width: 128px;" class="btn btn-light">
                                        <span><i class="ti ti-arrow-back"></i></span>&nbsp;Kembali
                                    </a>
                                @else
                                    <a href="{{ route('risk.process.monitoring.index') }}" style="min-width: 128px;"
                                        class="btn btn-light">
                                        <span><i class="ti ti-arrow-back"></i></span>&nbsp;Kembali
                                    </a>
                                @endif
                                ...
                                @isset($monitoring)
                                    @if (!str_contains(request()->route()->getName(), 'edit'))
                                        @if (
                                            ...
                                            <a href="{{ route('risk.process.monitoring.edit_monitoring', $monitoring->getEncryptedId()) }}"
