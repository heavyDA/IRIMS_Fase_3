...
@section('header-content')
    <div>
        <h1 class="page-title fw-medium fs-18 mb-2">Form Edit Kategori Risiko</h1>
        <div class="">
            <nav>
...
@section('main-content')
    <x-card>
        <x-slot name="body">
            <form method="POST" action="{{ route('master.risk_categories.update', $risk_category->getEncryptedId()) }}"
                class="row gap-2">
                @csrf
                @method('PATCH')
                <div class="col-12">
                    ...
                    <select name="type" class="form-select {{ $errors->has('type') ? 'is-invalid' : null }}" required>
                        ...
                        <option {{ old('type', $risk_category->type) == 'T2' ? 'selected' : null }} value="T2">T2
                        </option>
                        <option {{ old('type', $risk_category->type) == 'T3' ? 'selected' : null }} value="T3">T3
                        </option>
                    </select>
                    @error('type')
                ...
                <div class="col-12">
                    ...
                    <input value="{{ old('name', $risk_category->name) }}" name="name" type="text" required
