...
@section('header-content')
    <div>
        <h1 class="page-title fw-medium fs-18 mb-2">Form Tambah Posisi</h1>
        <div class="">
            <nav>
...
@section('main-content')
    <x-card>
        <x-slot name="body">
            <form method="POST" id="position-form" action="{{ route('setting.positions.store') }}" class="row gap-2">
                @csrf
                <div class="col-12">
                    <label for="sub_unit_code">Unit</label>
                    <select data-value="{{ old('sub_unit_code') }}" class="form-select" required name="sub_unit_code">
                ...
                <div class="col-12">
                    ...
                    <select class="form-select {{ $errors->has('roles') || $errors->has('roles.*') ? 'is-invalid' : null }}"
                        ...
                        <option value="risk admin" {{ in_array('risk admin', old('roles', [])) ? 'selected' : null }}>Risk
                            Admin</option>
                        <option value="risk owner" {{ in_array('risk owner', old('roles', [])) ? 'selected' : null }}>Risk
                            Owner</option>
                        <option value="risk otorisator"
                            {{ in_array('risk otorisator', old('roles', [])) ? 'selected' : null }}>
                            Risk Otorisator</option>
                        <option value="risk analis" {{ in_array('risk analis', old('roles', [])) ? 'selected' : null }}>Risk
                            Analis</option>
                        <option value="risk analis pusat"
                            {{ in_array('risk analis pusat', old('roles', [])) ? 'selected' : null }}>Risk
                            Analis Pusat</option>
                        <option value="risk reviewer"
                            {{ in_array('risk reviewer', old('roles', [])) ? 'selected' : null }}>
